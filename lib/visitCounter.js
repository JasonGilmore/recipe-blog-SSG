...
// Visits are not recorded if ?test=true or if is a css/js asset
// pathname includes search string
function middleware(req, res, next) {
    try {
        const url = req.path;
        const isTest = req.query.test === 'true' || req.body?.pathname?.includes('test=true');

        // Skip test traffic and assets
        if (isTest || url.startsWith('/css') || url.startsWith('/js')) {
            return next();
        }
        ...
        // Count specific site hits
        if (req.method === 'POST' && url == '/track-event' && req.body?.event == 'pageview' && req.body?.pathname) {
            const path = req.body.pathname.split('?')[0];
