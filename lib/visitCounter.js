...
function countPostVisit(contentType, postName) {
    if (!visitCounter[contentType]) {
        visitCounter[contentType] = {};
    }
    visitCounter[contentType][postName] = (visitCounter[contentType][postName] || 0) + 1;
}
...
function middleware(req, res, next) {
    try {
        ...
        // Only post count successful requests
        res.on('finish', () => {
            try {
                if (!(res.statusCode === 200 || res.statusCode === 304) || !isContentItem || !postName) {
                    return;
                }
                countPostVisit(matchedContentType, postName);
            } catch (err) {
                console.error('Error counting post visits (middleware):', err);
            }
        });
        next();
    } catch (err) {
        console.error('Error counting unique visits (middleware):', err);
        next();
    }
}
...
function setUniqueVisit(ip, userAgent) {
    ipSetToday.add(crypto.createHash('MD5').update(ip).update('|').update(userAgent).digest('hex'));
}
